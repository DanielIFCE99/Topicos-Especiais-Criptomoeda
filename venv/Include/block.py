...
class Blockchain:
    ...
    def generateNextBlock(self, data):
        ...
        if self.validatinBlock(newBlock) == True:
            self.__chain.append(newBlock)

    def validatinBlock(self, newBlock):
        previousBlock = self.getLatesBlock()
    ...
    def findBlock(self, index, previousHash, timestamp, data, difficulty, nonce):
        nonce = 0
        while True:
    ...
    def getDifficulty(self):
        latestBlock = self.getLatesBlock()
